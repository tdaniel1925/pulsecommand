...
export function PostModal({ post, onClose }: PostModalProps) {
  ...
  async function saveCaption() {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/dashboard/edit-post", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ postId: post.id, content: draft, platform: activeTab || undefined }),
      });
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto bg-neutral-50 p-5">
          ...
          {/* Caption — editable until the post is published */}
          <div className="mt-4 max-w-sm mx-auto">
            <div className="flex items-center justify-between mb-1.5">
              <p className="text-[10px] font-semibold text-neutral-400 uppercase tracking-wide">Caption text</p>
              {post.status !== "published" && !editing && (
                <button
                  onClick={() => { setDraft(activeCaption); setEditing(true); setSaved(false); }}
              ...
              {saved && !editing && (
                <span className="inline-flex items-center gap-1 text-[11px] font-medium text-green-600"><Check className="w-3 h-3" /> Saved</span>
              )}
            </div>
            {editing ? (
              <div className="space-y-2">
                <textarea
                  ...
                  rows={5}
                  className="w-full text-xs text-neutral-800 leading-relaxed bg-white border border-neutral-300 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none"
                />
                <div className="flex items-center gap-2">
                  <button
                    onClick={saveCaption}
                    disabled={saving}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-primary-600 rounded-lg hover:bg-primary-700 disabled:opacity-60"
                  >
                    {saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Check className="w-3.5 h-3.5" />} Save
                  </button>
                  <button onClick={() => setEditing(false)} className="px-3 py-1.5 text-xs font-medium text-neutral-500 hover:text-neutral-800">Cancel</button>
                </div>
              </div>
            ) : (
